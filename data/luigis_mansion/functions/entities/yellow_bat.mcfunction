execute if entity @s[tag=dead,tag=normal_death] run playsound luigis_mansion:entity.yellow_bat.vacuumed hostile @a ~ ~ ~ 1
execute if entity @s[tag=dead,tag=element_death] run playsound luigis_mansion:entity.yellow_bat.element_death hostile @a ~ ~ ~ 1
...
execute if entity @a[gamemode=!spectator,distance=..0.7,limit=1] run function luigis_mansion:entities/yellow_bat/collide
...
execute at @s[tag=!attack,tag=!fleeing] facing entity @p[gamemode=!spectator] feet rotated ~ 0 run teleport @s ~ ~ ~ ~ ~