...
execute if entity @e[distance=..1,tag=fire,limit=1] run scoreboard players set @s[scores={Time=1..2},tag=!burning] Time 1
execute if entity @s[scores={Time=1..2},tag=!burning,tag=!spoke] if entity @e[distance=..1,tag=fire,limit=1] run tellraw @a {"translate":"chat.type.text","with":[{"translate":"luigis_mansion:entity.shivers","color":"green"},{"translate":"luigis_mansion:message.shivers.fire"}]}
execute if entity @e[distance=..1,tag=fire,limit=1] run tag @s[scores={Time=1..2},tag=!burning] add burning
