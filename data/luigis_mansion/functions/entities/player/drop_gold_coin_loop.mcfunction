...
execute if score #temp2 LastTotalDamage matches 0 if score #temp Time matches 1.. positioned ~ ~2 ~ run function luigis_mansion:spawn_entities/item/gold_coin
execute if score #temp2 LastTotalDamage matches 0 if score #temp Time matches 1.. positioned ~ ~1 ~ as @e[tag=gold_coin,distance=..0.7] unless entity @s[scores={SpawnedTime=1..}] run scoreboard players set @s SpawnedTime 100
