...
execute if entity @s[scores={Time=40}] positioned ~ ~0.4 ~ as @e[distance=..0.1,tag=bone] unless entity @s[scores={GhostNr=1..}] positioned ~ ~-0.4 ~ run scoreboard players operation @s GhostNr = @e[tag=mr_bones,distance=..0.1] GhostNr
...
execute if entity @s[scores={Time=61}] run scoreboard players operation #temp GhostNr = @s GhostNr
execute if entity @s[scores={Time=61}] as @e[tag=bone] if score @s GhostNr = #temp GhostNr run tag @e[distance=..0.1] add wait
