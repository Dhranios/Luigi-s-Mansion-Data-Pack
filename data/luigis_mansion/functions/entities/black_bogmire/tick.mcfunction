...
execute if entity @s[tag=shadow_death] run playsound luigis_mansion:entity.black_bogmire.vacuumed hostile @a ~ ~ ~ 1
execute if entity @s[tag=shadow_death] positioned ~ ~0.5 ~ run function luigis_mansion:spawn_entities/ball/shadow
execute if entity @s[tag=shadow_death] run teleport @s ~ -100 ~
tag @s[tag=shadow_death] add dead
...
execute if entity @a[gamemode=!spectator,distance=..0.7,limit=1] if entity @s[tag=!dead,tag=!shadow_death,tag=!removed_from_existence] run function luigis_mansion:entities/black_bogmire/collide
