execute if entity @s[tag=dead] run playsound luigis_mansion:entity.spark.vacuumed hostile @a ~ ~ ~ 1
...
execute as @e[tag=spark,tag=spawn] if score @s Room = #temp Room run scoreboard players add #temp Time 1
execute unless score #temp Time matches 4.. unless entity @e[tag=spark,tag=spawn,distance=..0.7] run tag @s[scores={Room=1..}] add spawn
...
execute at @s[tag=!bounce,tag=spawn] if block ~ ~-0.01 ~ #luigis_mansion:ghosts_ignore run teleport @s ~ ~-0.01 ~
execute at @s[tag=bounce,tag=spawn] if block ~ ~0.04 ~ #luigis_mansion:ghosts_ignore run teleport @s ~ ~0.01 ~
execute if block ~ ~-0.07 ~ #luigis_mansion:ghosts_ignore run tag @s remove bounce
execute unless block ~ ~-0.1 ~ #luigis_mansion:ghosts_ignore run tag @s add bounce
