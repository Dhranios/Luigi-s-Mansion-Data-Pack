...
execute at @s[tag=!element_hurt,tag=!fleeing,tag=!collided,scores={StunTime=0}] facing entity @p[gamemode=!spectator] feet rotated ~ 0 run teleport @s ~ ~ ~ ~ ~
execute if entity @s[tag=!element_hurt,tag=!fleeing,tag=!collided,tag=laugh,scores={StunTime=0}] run function luigis_mansion:entities/ceiling_surprise/laugh
