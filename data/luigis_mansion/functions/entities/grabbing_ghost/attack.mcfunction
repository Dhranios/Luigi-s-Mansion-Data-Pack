...
execute if entity @s[scores={ActionTime=21..41}] positioned ^ ^ ^0.8 store result entity @s Rotation[0] float 0.01 run data get entity @a[gamemode=!spectator,tag=grabbed,limit=1] Rotation[0] 100
...
execute if entity @s[scores={ActionTime=21..41}] positioned ^ ^ ^0.8 positioned as @a[gamemode=!spectator,distance=..0.7,tag=grabbed,limit=1] rotated as @s positioned ^ ^ ^-0.8 rotated as @s run teleport @s ~ ~ ~ ~ ~
...
execute if entity @s[scores={ActionTime=42..62}] run function luigis_mansion:animations/grabbing_ghost/let_go
