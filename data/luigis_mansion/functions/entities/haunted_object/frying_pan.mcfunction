execute if entity @s[tag=dead,tag=normal_death] run playsound luigis_mansion:entity.haunted_frying_pan.vacuumed hostile @a ~ ~ ~ 1
execute if entity @s[tag=dead,tag=element_death] run playsound luigis_mansion:entity.haunted_frying_pan.element_death hostile @a ~ ~ ~ 1
...
execute if entity @s[scores={WaitTime=100..},tag=moving] run function luigis_mansion:animations/haunted_object/move_horizontal