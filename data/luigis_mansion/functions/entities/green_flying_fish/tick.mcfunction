...
execute unless score #temp Time matches 3.. unless entity @e[tag=green_flying_fish,tag=spawn,distance=..0.7] if entity @s[tag=!spawn] run playsound luigis_mansion:entity.green_flying_fish.spawn hostile @a ~ ~ ~ 1
execute unless score #temp Time matches 3.. unless entity @e[tag=green_flying_fish,tag=spawn,distance=..0.7] run tag @s add spawn
