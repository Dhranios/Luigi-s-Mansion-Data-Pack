...
execute unless entity @s[scores={Time=-20}] run scoreboard players remove @s[tag=!fleeing,tag=!attack,tag=!collided,tag=!laugh,tag=!complain,scores={StunTime=0}] Time 1
execute if entity @a[gamemode=!spectator,distance=..0.7,limit=1] run function luigis_mansion:entities/mr_bones/collide
execute if entity @s[tag=!fleeing,tag=!attack,tag=!collided,tag=!laugh,tag=!complain,scores={StunTime=0,Time=-20}] facing entity @p[gamemode=!spectator] feet rotated ~ 0 run function luigis_mansion:entities/mr_bones/line_of_sight
...
execute at @s[tag=!fleeing,tag=!attack,tag=!collided,tag=!laugh,tag=!complain,scores={StunTime=0}] facing entity @p[gamemode=!spectator] feet rotated ~ 0 run teleport @s ~ ~ ~ ~90 ~
execute at @s[tag=!fleeing,tag=!attack,tag=!collided,scores={StunTime=0}] unless entity @s[tag=!laugh,tag=!complain] facing entity @p[gamemode=!spectator] feet rotated ~ 0 run teleport @s ~ ~ ~ ~ ~
