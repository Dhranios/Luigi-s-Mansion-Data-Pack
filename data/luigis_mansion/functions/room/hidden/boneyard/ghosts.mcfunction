...
execute unless data storage luigis_mansion:data current_state.current_data.room.boneyard{cleared:1b} if entity @a[tag=!blackout] unless score #boneyard Wave matches 1.. run function luigis_mansion:room/hidden/boneyard/wave_1
...
execute unless score #boneyard_water Searched matches 1 if entity @a[x=681.5,y=102,z=-24.5,distance=..5] if entity @a[nbt={Inventory:[{tag:{luigis_mansion:{id:"luigis_mansion:water_element_medal"}}}]}] unless entity @e[x=681.5,y=102,z=-24.5,distance=..5,tag=water_elemental_ghost] positioned 681 102 -25 run function luigis_mansion:spawn_entities/ghost/water_elemental_ghost