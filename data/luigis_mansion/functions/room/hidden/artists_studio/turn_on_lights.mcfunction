execute unless entity @a[nbt={Inventory:[{tag:{luigis_mansion:{id:"luigis_mansion:key",room:"secret_altar"}}}]},limit=1] if entity @a[advancements={luigis_mansion:mansion/secret_altar_key=false},limit=1] run summon minecraft:item 696 121 50 {Motion:[0.0d,0.3d,0.1d],Item:{id:"minecraft:brick",Count:1b,tag:{CustomModelData:5,luigis_mansion:{id:"luigis_mansion:key",room:"secret_altar"},display:{Name:'{"italic":false,"color":"white","translate":"luigis_mansion:item.key","with":[{"translate":"luigis_mansion:location.secret_altar"}]}'}}},Age:-32768s}
...
execute if entity @a[advancements={luigis_mansion:technical={released_boos_talk=true}},scores={Boos=..49},limit=1] run function luigis_mansion:room/hidden/artists_studio/load_boos
