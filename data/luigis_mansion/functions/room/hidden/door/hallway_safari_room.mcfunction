...
execute if score #temp Searched matches 1..2 as @a[gamemode=!spectator,distance=..3,x=706.0,y=120,z=36.5,nbt={SelectedItem:{tag:{luigis_mansion:{id:"luigis_mansion:key",room:"safari_room"}}}}] run function luigis_mansion:room/hidden/safari_room/unlock_door
execute if block 705 120 36 #minecraft:doors[open=true] if entity @a[advancements={luigis_mansion:mansion/safari_room_key=false},limit=1] positioned 705 120 36 run function luigis_mansion:blocks/closed_door
execute if entity @a[advancements={luigis_mansion:mansion/safari_room_key=false},limit=1] if score #temp Searched matches 1..2 run scoreboard players set #temp Searched -1
