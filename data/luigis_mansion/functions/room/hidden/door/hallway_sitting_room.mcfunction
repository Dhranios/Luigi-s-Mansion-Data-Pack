...
execute if score #temp Searched matches 1..2 as @a[gamemode=!spectator,distance=..3,x=706.0,y=111,z=32.5,nbt={SelectedItem:{tag:{luigis_mansion:{id:"luigis_mansion:key",room:"sitting_room"}}}}] run function luigis_mansion:room/normal/sitting_room/unlock_door
execute if block 705 111 32 #minecraft:doors[open=true] if entity @a[advancements={luigis_mansion:mansion/sitting_room_key=false},limit=1] positioned 705 111 32 run function luigis_mansion:blocks/closed_door
execute if entity @a[advancements={luigis_mansion:mansion/sitting_room_key=false},limit=1] if score #temp Searched matches 1..2 run scoreboard players set #temp Searched -1
