...
execute if score #temp Searched matches 1..2 as @a[gamemode=!spectator,distance=..3,x=706.0,y=102,z=30.5,nbt={SelectedItem:{tag:{luigis_mansion:{id:"luigis_mansion:key",room:"fortune_tellers_room"}}}}] run function luigis_mansion:room/normal/fortune_tellers_room/unlock_door
execute if block 705 102 30 #minecraft:doors[open=true] if entity @a[advancements={luigis_mansion:mansion/fortune_tellers_room_key=false},limit=1] positioned 705 102 30 run function luigis_mansion:blocks/closed_door
