...
execute if block 681 104 47 minecraft:stone_button[powered=true] if block 674 102 39 minecraft:andesite_stairs unless entity @a[tag=moving_wall,limit=1] if entity @a[advancements={luigis_mansion:mansion/storage_room=true},limit=1] positioned 681 104 47 run tag @p[gamemode=!spectator] add move_wall
execute if block 673 104 33 minecraft:stone_button[powered=true] if block 677 101 33 minecraft:oak_slab unless entity @a[tag=release_boos,limit=1] positioned 673 104 33 run tag @p[gamemode=!spectator] add release_boos
