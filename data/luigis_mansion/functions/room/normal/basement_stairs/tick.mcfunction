...
execute as @e[type=!minecraft:item_frame,x=698,y=93,z=-42,dx=8,dy=15,dz=19] run scoreboard players set @s Room 11
tag @e[tag=ghost,scores={Room=11}] add hallway

execute as @a[gamemode=!spectator,x=700,y=93,z=-40,dx=4,dy=15,dz=15] run function luigis_mansion:room/normal/basement_stairs/tick_per_player

execute if entity @a[tag=blackout] unless score #basement_stairs Wave matches 1.. run function luigis_mansion:room/normal/basement_stairs/blackout
