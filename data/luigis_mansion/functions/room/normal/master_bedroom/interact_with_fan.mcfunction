execute if entity @s[x=691.0,y=117,z=47.0,distance=..1.5,tag=vacuum] unless score #master_bedroom_fan Search matches 1 unless score #master_bedroom_fan Searching matches 201.. run scoreboard players add #master_bedroom_fan Searching 1
execute if entity @s[x=691.0,y=117,z=47.0,distance=..1.5,tag=vacuum] run scoreboard players set #master_bedroom_fan Search 1
...
execute if entity @s[x=691.0,y=117,z=47.0,distance=..1.5,tag=gameboy_horror_scan] run tellraw @a {"translate":"chat.type.text","with":[{"selector":"@p[tag=scanning_player,gamemode=!spectator]","color":"green"},{"translate":"luigis_mansion:message.player.scan_furniture.21"}]}