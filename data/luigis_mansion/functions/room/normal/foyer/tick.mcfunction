...
execute if entity @a[tag=blackout] unless score #foyer Wave matches 1.. run function luigis_mansion:room/normal/foyer/blackout
...
execute unless entity @e[type=minecraft:item,nbt={Item:{tag:{luigis_mansion:{id:"luigis_mansion:key",room:"parlor"}}}},limit=1] unless entity @e[type=minecraft:armor_stand,tag=first_key,limit=1] unless entity @a[nbt={Inventory:[{tag:{luigis_mansion:{id:"luigis_mansion:key",room:"parlor"}}}]},limit=1] if entity @a[advancements={luigis_mansion:mansion/parlor_key=false},limit=1] if entity @e[x=699,y=111,z=7,dx=0,dy=1,dz=1,tag=door] run summon minecraft:armor_stand 714.0 104 8.0 {Tags:["first_key"],NoGravity:1b,Invulnerable:1b,Silent:1b,Invisible:1b,DisabledSlots:2039583,ArmorItems:[{},{},{},{id:"minecraft:brick",Count:1b,tag:{CustomModelData:1}}]}
