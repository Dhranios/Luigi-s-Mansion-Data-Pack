execute if entity @s[x=679.0,y=108,z=26.0,distance=..1.5,tag=vacuum] unless score #billiards_room_fan Search matches 1 unless score #billiards_room_fan Searching matches 201.. run scoreboard players add #billiards_room_fan Searching 1
execute if entity @s[x=679.0,y=108,z=26.0,distance=..1.5,tag=vacuum] run scoreboard players set #billiards_room_fan Search 1
...
execute if entity @s[x=679.0,y=108,z=26.0,distance=..1.5,tag=gameboy_horror_scan] run tellraw @a {"translate":"chat.type.text","with":[{"selector":"@p[tag=scanning_player,gamemode=!spectator]","color":"green"},{"translate":"luigis_mansion:message.player.scan_furniture.104"}]}