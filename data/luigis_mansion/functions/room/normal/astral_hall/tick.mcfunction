...
execute unless data storage luigis_mansion:data current_state.current_data.room.astral_hall{cleared:1b} unless block 677 111 -32 minecraft:iron_door[powered=true] run scoreboard players set #temp Searched 1
...
execute as @a[x=677,y=111,z=-31,dx=0,dy=1,dz=0] unless data storage luigis_mansion:data current_state.current_data.room.astral_hall{cleared:1b} at @s run function luigis_mansion:room/normal/astral_hall/trick_door
