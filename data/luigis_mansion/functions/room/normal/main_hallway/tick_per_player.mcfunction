...
execute if entity @s[x=704,y=102,z=-6,distance=..9,tag=!chance_check] unless data storage luigis_mansion:data current_state.current_data.money_spawned{main_hallway_gold_mouse:1b} unless entity @e[tag=gold_mouse,scores={Room=10},limit=1] run function luigis_mansion:room/normal/main_hallway/gold_mouse
tag @s[x=704,y=102,z=-6,distance=9..10,tag=chance_check] remove chance_check

execute at @s unless block ~ ~ ~ #minecraft:doors unless data storage luigis_mansion:data current_state.current_data.rooms.conservatory{cleared:1b} run playsound luigis_mansion:music.mansion.melody hostile @s[x=666,y=102,z=-15,dx=4,dy=6,dz=39,scores={HallwayNoise=0}] ~ ~ ~ 1000
execute at @s unless block ~ ~ ~ #minecraft:doors unless data storage luigis_mansion:data current_state.current_data.rooms.conservatory{cleared:1b} run scoreboard players set @s[x=666,y=102,z=-15,dx=4,dy=6,dz=39,scores={HallwayNoise=0}] HallwayNoise 850
...
execute at @s unless block ~ ~ ~ #minecraft:doors unless data storage luigis_mansion:data current_state.current_data.rooms.dining_room{cleared:1b} if entity @s[x=685,y=102,z=-8,dx=14,dy=6,dz=4] at @s run function luigis_mansion:room/normal/dining_room/noise
