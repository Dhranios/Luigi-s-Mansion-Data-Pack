...
execute if score #temp Searched matches 1..2 as @a[gamemode=!spectator,distance=..3,x=694.5,y=120,z=-16.0,nbt={SelectedItem:{tag:{luigis_mansion:{id:"luigis_mansion:key",room:"artists_studio"}}}}] run function luigis_mansion:room/normal/artists_studio/unlock_door
execute if block 694 120 -16 #minecraft:doors[open=true] if entity @a[advancements={luigis_mansion:mansion/artists_studio_key=false},limit=1] positioned 694 120 -16 run function luigis_mansion:blocks/closed_door
execute if entity @a[advancements={luigis_mansion:mansion/artists_studio_key=false},limit=1] if score #temp Searched matches 1..2 run scoreboard players set #temp Searched -1
