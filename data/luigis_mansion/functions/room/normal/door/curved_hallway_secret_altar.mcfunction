...
execute if block 653 93 -18 #minecraft:doors[open=true] if entity @a[advancements={luigis_mansion:mansion/secret_altar=true},limit=1] unless entity @e[type=minecraft:item_frame,nbt={Item:{id:"minecraft:painting"}},limit=1] positioned 653 93 -18 run function luigis_mansion:blocks/closed_door
...
execute if entity @a[advancements={luigis_mansion:mansion/secret_altar=true},limit=1] unless entity @e[type=minecraft:item_frame,nbt={Item:{id:"minecraft:painting"}},limit=1] if score #temp Searched matches 1..2 run scoreboard players set #temp Searched -1
