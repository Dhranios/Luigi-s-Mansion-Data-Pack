...
execute if score #temp Searched matches 1..2 as @a[gamemode=!spectator,distance=..3,x=672.0,y=102,z=18.5] if data storage luigis_mansion:data current_state.current_data.obtained_keys{billiards_room:1b} unless data storage luigis_mansion:data current_state.current_data.used_keys{billiards_room:1b} run function luigis_mansion:room/normal/billiards_room/unlock_door
execute if block 671 102 18 #minecraft:doors[open=true] unless data storage luigis_mansion:data current_state.current_data.used_keys{billiards_room:1b} positioned 671 102 18 run function luigis_mansion:blocks/closed_door
