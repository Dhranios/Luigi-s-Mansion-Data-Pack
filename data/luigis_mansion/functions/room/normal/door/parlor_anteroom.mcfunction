...
execute if score #temp Searched matches 1..2 as @a[gamemode=!spectator,distance=..3,x=683.0,y=111,z=1.5] if data storage luigis_mansion:data current_state.current_data.obtained_keys{anteroom:1b} unless data storage luigis_mansion:data current_state.current_data.used_keys{anteroom:1b} run function luigis_mansion:room/normal/anteroom/unlock_door
execute if block 683 111 1 #minecraft:doors[open=true] unless data storage luigis_mansion:data current_state.current_data.used_keys{anteroom:1b} positioned 683 111 1 run function luigis_mansion:blocks/closed_door
