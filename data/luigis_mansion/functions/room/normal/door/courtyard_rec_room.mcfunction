...
execute if score #temp Searched matches 1..2 as @a[gamemode=!spectator,distance=..3,x=654.0,y=102,z=-37.5,nbt={SelectedItem:{tag:{luigis_mansion:{id:"luigis_mansion:key",room:"rec_room"}}}}] run function luigis_mansion:room/normal/rec_room/unlock_door
...
execute unless entity @a[gamemode=!spectator,distance=..3,x=654.0,y=102,z=-37.5] if score #temp Searched matches 1..2 run scoreboard players set #temp Searched 0
