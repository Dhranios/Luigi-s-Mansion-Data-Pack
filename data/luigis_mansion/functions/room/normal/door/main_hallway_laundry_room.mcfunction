...
execute if score #temp Searched matches 1..2 as @a[gamemode=!spectator,distance=..3,x=702.5,y=102,z=38.0] if data storage luigis_mansion:data current_state.current_data.obtained_keys{laundry_room:1b} unless data storage luigis_mansion:data current_state.current_data.used_keys{laundry_room:1b} run function luigis_mansion:room/normal/laundry_room/unlock_door
execute if block 702 102 37 #minecraft:doors[open=true] unless data storage luigis_mansion:data current_state.current_data.used_keys{laundry_room:1b} positioned 702 102 37 run function luigis_mansion:blocks/closed_door
