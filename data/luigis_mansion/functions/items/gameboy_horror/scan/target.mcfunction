...
execute unless entity @s[tag=warp] unless entity @e[tag=target,limit=1] positioned ^ ^ ^0.5 if score #temp Move matches ..20 run function luigis_mansion:items/gameboy_horror/scan/target