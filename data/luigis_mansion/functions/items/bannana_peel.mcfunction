execute if entity @e[distance=..0.7,type=minecraft:armor_stand,tag=bannana_peel,tag=!dead] run tag @s add tripping
execute if entity @s[tag=tripping,tag=!tripping_2] run kill @e[distance=..0.7,type=minecraft:armor_stand,tag=bannana_peel,limit=1]
