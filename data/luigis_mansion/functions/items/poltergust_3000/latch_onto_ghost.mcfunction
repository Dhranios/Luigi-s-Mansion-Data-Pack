...
execute if entity @s[tag=!already_included] as @e[distance=..0.7,tag=ghost,scores={VulnerableTime=0..}] if score @s Room = #temp Room unless data entity @s ArmorItems[3] run data merge entity @s {ArmorItems:[{},{},{},{id:"minecraft:structure_void",Count:1b,tag:{attacked_by:[]}}]}
