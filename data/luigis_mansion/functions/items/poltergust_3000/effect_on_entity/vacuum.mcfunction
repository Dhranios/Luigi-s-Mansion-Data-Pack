...
execute if entity @s[tag=vacuumable,tag=!boolossus] unless entity @s[tag=wool,scores={SpawnTime=..19}] unless entity @s[tag=billiards_ball,scores={SpawnTime=..19}] unless entity @s[tag=ball,tag=spit] unless entity @s[tag=haunted_object,tag=!moving,tag=!haunted_music_sheet] positioned as @s if block ^ ^ ^0.3 #luigis_mansion:ghosts_ignore unless entity @a[gamemode=!spectator,distance=..1.2,limit=1] run teleport @s ^ ^ ^0.1
