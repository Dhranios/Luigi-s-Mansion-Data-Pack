...
execute if entity @s[nbt={Inventory:[{tag:{luigis_mansion:{id:"luigis_mansion:key",room:"attic_hallway_2"}}}]},advancements={luigis_mansion:technical={area_3_key=false}}] run scoreboard players set @s GBHCall 10
execute if entity @s[scores={GBHCall=1..}] as @a unless entity @s[scores={MusicType=19}] unless entity @s[scores={MusicType=27}] run function luigis_mansion:other/music/set/gameboy_horror_calling
